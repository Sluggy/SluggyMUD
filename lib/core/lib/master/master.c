...
// contains all the valid_* functions
inherit VALID_OBJ;
...
object connect()
{
    ...
    err = catch(login_ob = new(LOGIN_OBJ));

    if (err) {
        write("The core login object is being worked on. No one can log into the mud at this time.\n");
        write(err);
...
{
    write_file(PUBLIC_LOG_DIR + "/compile", message);
}
...
{
    return (string)call_other(SEFUN_OBJ, "creator_file", str);
}

string domain_file(string str)
{
    return (string)call_other(SEFUN_OBJ, "domain_file", str);
}

string author_file(string str)
{
    return (string)call_other(SEFUN_OBJ, "author_file", str);
}
