...
/**
 *
 * TODO: Make the commandHook and exec_comm functions pass
 * the args of the command directly to the create() function.
 */
...
void exec_command(string arg)
{
    ...
    cobj = load_object(PUBLIC_VERBS_DIR + verb);
    if(!cobj) cobj = load_object(SECURE_VERBS_DIR + verb);
    if(!cobj) cobj = load_object(PUBLIC_CMDS_DIR + verb);
    if(!cobj) cobj = load_object(SECURE_CMDS_DIR + verb);
...
int commandHook(string arg)
{
    ...
    //we are going to look in both public and secure commands and verbs directories
    //cmd_path = PUBLIC_CMDS_DIR + query_verb();

    cobj = load_object(PUBLIC_CMDS_DIR + verb);
    if(!cobj) cobj = load_object(PUBLIC_VERBS_DIR + verb);
    if(!cobj) cobj = load_object(SECURE_CMDS_DIR + verb);
    if(!cobj) cobj = load_object(SECURE_VERBS_DIR + verb);
    
    if (cobj) return (int)cobj->main(arg);
